...
def greekify(text):
    ...
    output = ""
    for i in text: 
        if i in latin_alphabet:
            index = latin_alphabet.index(i)
            output = output + greek_alphabet[index]
        elif i in greek_alphabet:
            index = greek_alphabet.index(i)
            output = output + latin_alphabet[index]
        else:
            output = output + i
    return output 
...
def grapheme_equality(x, y):
    ...
    y = unicodedata.normalize('NFKD',y) 
...
# for i in GoogleTranslator(source='en', target='es').translate("python is awesome")
#     print(GoogleTranslator(source='en', target='i').translate("python is awesome"))

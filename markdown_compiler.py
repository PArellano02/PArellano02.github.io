...
def compile_headers(line):
    ...
    accumulator = ''
    header_count = line.split(' ')
    first = header_count[0]
    line2 = ''
    for c in line: 
        if c != '#':
            line2 += c
            ...
            line2 += line2
    if '#' in first:
        accumulator += '<h'+str(len(first))+'>'+ line2 + '</h'+str(len(first))+'>'
        return accumulator
    else:
        return line 
...
def compile_code_inline(line):
    ...
    for c in line :
        if c == '<' and inline_code == True:
            ...
            tilde_count -= 1
            if inline_code == False and tilde_count >= 1 :
                accumulator += '<code>'
            if inline_code == False and tilde_count < 1 :
                accumulator += c
            if inline_code == True:
...
def compile_links(line):
    ...
    '''
    accumulator = ''
    index_bopen = line.find('[')
    index_bclose = line.find(']')
    index_popen =line.find('(')
    index_pclose = line.find(')')
    # if index_bopen <
    if index_popen > -1 and index_bopen > -1 and index_bclose > -1 and index_pclose > -1:
        if index_bopen < index_bclose and index_bclose +1 == index_popen and index_popen < index_pclose:
            accumulator += line[0:index_bopen] + '<a href="'+ line[index_popen+1:index_pclose]+'">'+ line[index_bopen+1:index_bclose] + '</a>' + line[index_pclose+1:]
            return accumulator
        else:
            return line
    else:
...
def compile_images(line):
    ...
    index_popen =line.find('(')
    index_pclose = line.find(')')
    if index_popen > -1 and index_bopen > -1 and index_bclose > -1 and index_pclose > -1 and bang > -1 :
        if bang < index_bopen and index_bopen < index_bclose and index_bclose +1 == index_popen and index_popen < index_pclose:
            accumulator += line[0:bang] + '<img src="'+ line[index_popen+1:index_pclose]+'" '+ 'alt="' + line[index_bopen+1:index_bclose] + '" />' + line[index_pclose+1:]
            return accumulator
        else:
            return line
    else:
